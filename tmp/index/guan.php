...
$asql=$this->Sql("select  `s_content`.*,`s_user`.`nickname`,`s_user`.`face`".$likes1." from   `s_content` left join `s_user` using(`uid`)  ".$likes."  left join `s_follow` on(`s_content`.`uid`=`s_follow`.`uid2` ) where `s_follow`.`uid`='".$_SESSION["uid"]."'    order  by   `s_content`.`cid`  desc   limit ".$pnno.",".($p+1));
$lnum=$this->num($asql);
